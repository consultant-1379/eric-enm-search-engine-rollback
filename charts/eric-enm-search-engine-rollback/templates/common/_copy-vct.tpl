...
*/ -}}
{{- define "eric-enm-search-engine-rollback.copyVCTMetadataFields" -}}
...
*/ -}}
{{- define "eric-enm-search-engine-rollback.copyVCTAnnotations" -}}
  {{- include "eric-enm-search-engine-rollback.copyVCTMetadataFields" (dict "metadataField" "annotations" "vctName" .vctName "customKeys" .customKeys "release" .release "name" .name) | trim -}}
{{- end -}}

{{- define "eric-enm-search-engine-rollback.copyVCTLabels" -}}
  {{- $defaultLabels := dict "app.kubernetes.io/instance" .release.Name -}}
  {{- $mergedLabels := include "eric-enm-search-engine-rollback.mergeLabels" (dict "location" (printf "%s.copyVCT" .name) "sources" (list $defaultLabels .customKeys)) | fromYaml -}}
  {{- include "eric-enm-search-engine-rollback.copyVCTMetadataFields" (dict "metadataField" "labels" "vctName" .vctName "customKeys" $mergedLabels "release" .release "name" .name) | trim -}}
{{- end -}}
